...
module.exports = {
    importChar: function(region, name, realm, fn) {
        ...
        if(region === '' || name === '' || realm === ''){
            ...
            var statsUrl = url + '?fields=stats,items,talents&locale=en_US&jsonp=data&apikey=fbgams9zxkqsezwqaavxxk9u8rkvxxkn';
			console.log(url);
...
//            var statsUrl = 'http://us.battle.net/api/wow/character/Stormreaver/Calligraphy?fields=stats,items,talents&locale=en_US&jsonp=callback&apikey=fbgams9zxkqsezwqaavxxk9u8rkvxxkn';
            // swapping to using request. built-in NPM stuff requires me to update node.
			request(statsUrl, function(error, response, body){
				if(!error && response.statusCode == 200) {
					var f = new Function("data", body);
					
					// ty jsonp for being a poop
					f(function(json) {
						fn(json);
					});
				}
    ...
    //    // 6.0 stats?
		...
		this.mainDpsAvgd = stats["mainHandDps"] || 0;
